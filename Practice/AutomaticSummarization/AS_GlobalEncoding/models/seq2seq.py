...
class seq2seq(nn.Module):
    ...
    def sample(self, src, src_len):
        ...
        outputs = torch.stack(outputs)
        print(f'outputs in sample: {outputs}')
        ...
        print(f'sample_ids in sample: {sample_ids}')
    ...
    def beam_sample(self, src, src_len, beam_size=1, eval_=False):
        ...
        # 根据src的句子长度 按列降序排序
        lengths, indices = torch.sort(src_len, dim=0, descending=True)
        # 对indices按列升序（默认descending=False）排序
        _, ind = torch.sort(indices)
        src = torch.index_select(src, dim=0, index=indices)

        src = src.t()
        batch_size = src.size(1)
        print(f"src.size(0): {src.size(0)}")    # 45
        print(f"src.size(1): {src.size(1)}")    # 101
        ...
        print(f"len(contexts): {len(contexts)}")    # 45
        print(f"len(encState): {len(encState)}")    # 2
