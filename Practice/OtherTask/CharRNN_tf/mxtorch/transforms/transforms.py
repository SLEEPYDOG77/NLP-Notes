...
class ToPILImage(object):
    ...
    Args:
        mode (`PIL.Image mode`_): color space and pixel depth of input 0data (optional).
            If ``mode`` is ``None`` (default) there are some assumptions made about the input 0data:
            1. If the input has 3 channels, the ``mode`` is assumed to be ``RGB``.
            2. If the input has 4 channels, the ``mode`` is assumed to be ``RGBA``.
            3. If the input has 1 channel, the ``mode`` is determined by the 0data type (i,e,
            ``int``, ``float``, ``short``).
...
class LinearTransformation(object):
    ...
    Applications:
    - whitening: zero-center the 0data, compute the 0data covariance matrix
