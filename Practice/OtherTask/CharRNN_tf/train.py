...
from utils.dataset import TextDataset, TextConverter
...
class CharRNNTrainer(Trainer):
    ...
    def train(self, kwargs):
        ...
        for data in tqdm(train_data):
            ...
            # self.metric_meter['loss'].add(loss.0data[0])
            self.metric_meter['loss'].add(loss)

            # Update to tensorboard.
            if (self.n_iter + 1) % opt.plot_freq == 0:
                self.writer.add_scalar('perplexity', torch.log(self.metric_meter['loss'].value()[0]), self.n_plot)
                self.n_plot += 1

            self.n_iter += 1

        # Log the train metrics to dict.
        loss = self.metric_meter['loss'].value()[0]
        self.metric_log['perplexity'] = torch.log(loss)
